...
struct stuff * makeStuff(char * data)
{
  ...
  if(mStuff == NULL)
    ...
    {
      /* Set data to members */
      mStuff->insides = (char*)malloc((strlen(data)+1)*sizeof(char));
      strcpy(mStuff->insides, data);
...
{
  /* loop over ellemt of struct and free them*/
  int i;
  for(i=0; i<size; i++)
    {
      free(myArray[i].insides);
      myArray->insides = NULL;
    }
